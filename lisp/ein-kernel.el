...
(defun ein:kernel-start-channels (kernel)
    ...
    (let* ((ws-url (concat (ein:$kernel-ws-url kernel)
      ...
      (if (string-prefix-p "http" ws-url)
          ;; SockJS (https://github.com/ipython/ipython/pull/2321)
          (let ((real-ws-url (destructuring-bind (protocol rest)
                                 (split-string ws-url "://")
                               (concat "ws://" rest))))
            (setf (ein:$kernel-shell-channel kernel)
                  (ein:websocket (concat real-ws-url "/shell/websocket")))
            (setf (ein:$kernel-iopub-channel kernel)
                  (ein:websocket (concat real-ws-url "/iopub/websocket"))))
        (setf (ein:$kernel-shell-channel kernel)
